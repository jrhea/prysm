...
// RunForkTransitionTest is a helper function that runs electra's transition core tests.
func RunForkTransitionTest(t *testing.T, config string) {
	params.SetupTestConfigCleanup(t)
	require.NoError(t, utils.SetConfig(t, config))

	testFolders, testsFolderPath := utils.TestFolders(t, config, "electra", "transition/core/pyspec_tests")
	for _, folder := range testFolders {
		t.Run(folder.Name(), func(t *testing.T) {
			...
			preforkBlocks := make([]*ethpb.SignedBeaconBlockDeneb, 0)
			postforkBlocks := make([]*ethpb.SignedBeaconBlockElectra, 0)
			...
			bc.ElectraForkEpoch = types.Epoch(config.ForkEpoch)
